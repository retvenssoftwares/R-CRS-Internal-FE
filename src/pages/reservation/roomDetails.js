...
const RoomDetails = (props) => {
  ...
  const extras = [
    {
      ExtraChargeId: "645",
      ShortCode: "Bottle of Wine on Arrival",
      charge: "Bottle of Wine on Arrival",
      description: null,
      Rate: "500.0000",
      ChargeRule: "PERQUANTITY",
      PostingRule: "ONLYCHECKOUT",
      ValidFrom: null,
      ValidTo: null,
      ischargealways: "0",
      applyon_rateplan: "XXXXXXXXXXXXXXXXXX",
      applyon_special: "",
    },
    {
      ExtraChargeId: "245",
      ShortCode: "Transport",
      charge: "Transport Services",
      description: null,
      Rate: "500.0000",
      ChargeRule: "PERBOOKING",
      PostingRule: "ONLYCHECKIN",
      ValidFrom: null,
      ValidTo: null,
      ischargealways: "0",
      applyon_rateplan: "ALL",
      applyon_special: "ALL",
    },
    {
      ExtraChargeId: "24354",
  ...
  useEffect(() => {
    ...
    const filteredPlans = roomInfo.RoomInfo.RatePlans.RatePlan.filter(
      (ratePlan) =>
        ratePlan.RoomTypeID === reservation.roomTypes[index].room_type_id
    );
  ...
  useEffect(() => {
    if (reservation.roomTypes[index].room_type_id) {
      const availableRoomType = availableRoom.Success.RoomList.find(
        (roomType) =>
          roomType.RoomtypeID === reservation.roomTypes[index].room_type_id
      );
            ...
            >
              ...
              >
                ...
                {selectedItems.map((itemId) => {
                  const selectedItem = extras.find(
                    (item) => item.ExtraChargeId === itemId
                  );
                  return (
                    <li key={itemId}>
  ...
  function calculateTotalPricee() {
    ...
    const roomInfo = roomRate.RES_Response.RoomInfo.Source.find(
      (source) =>
        source.RoomTypes.RateType.RoomTypeID ===
          reservation.roomTypes[index].room_type_id &&
        source.RoomTypes.RateType.RateTypeID ===
          reservation.roomTypes[index].rate_type_id
    );
    ...
    const totalPrice =
      (basePrice +
        extraAdultPrice *
          Math.max(reservation.roomTypes[index].adult_number - 2, 0) +
        extraChildPrice *
          Math.max(reservation.roomTypes[index].child_number, 0)) *
      reservation.roomTypes[index].number_of_rooms;
          ...
          <Grid item xs={12} md={6}>
            <FormControl fullWidth>
              <TextField
                ...
                helperText={
                  reservation.roomTypes[index].adult_number > 3
                    ? "Exceeds maximum adults"
                    : ""
                }
              />
            </FormControl>
          ...
          <Grid item xs={12} md={6}>
            <FormControl fullWidth>
              <TextField
                ...
                helperText={
                  reservation.roomTypes[index].child_number > 1
                    ? "Exceeds maximum chidren"
                    : ""
                }
              />
            </FormControl>
